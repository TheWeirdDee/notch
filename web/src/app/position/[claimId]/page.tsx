...
export default function PositionPage({params}:{params:Promise<{claimId:string}>}) {
  ...
  return <div className="mx-auto max-w-6xl px-5 py-10 sm:px-10 sm:py-14">
    ...
    <div className="mt-6 grid items-start gap-6 lg:grid-cols-[1.5fr_1fr]"><div className="min-w-0 space-y-6">
      <section className="panel p-5 sm:p-7"><h2 className="font-display text-2xl">Source & ownership</h2>{claim ? <dl className="mt-5 divide-y divide-line text-sm"><div className="flex flex-wrap justify-between gap-3 py-3"><dt className="text-ink-soft">Sablier stream</dt><dd>#{claim.streamId.toString()}</dd></div><div className="flex flex-wrap justify-between gap-3 py-3"><dt className="text-ink-soft">Borrower</dt><dd>{shortAddress(claim.borrower)}</dd></div><div className="flex flex-wrap justify-between gap-3 py-3"><dt className="text-ink-soft">Lock expires</dt><dd>{new Date(Number(claim.lockedUntil)*1000).toLocaleString()}</dd></div><div className="pt-4"><a href={SEPOLIA_EXPLORER_TX(claim.sourceTx)} target="_blank" rel="noreferrer" className="underline">View source on Sepolia</a></div></dl> : <p className="mt-5 text-sm text-ink-soft">Awaiting a live registry read.</p>}</section>
