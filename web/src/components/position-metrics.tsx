...
export function PositionMetrics({ claimId }: { claimId?: string }) {
  ...
  const p = query.data;
  return <div className="panel p-5 sm:p-7" data-testid="live-metrics">
    ...
    <dl className="mt-6 grid gap-6 sm:grid-cols-3">{[["Verified capacity", p.originalCapacity], ["Financed", p.financedCapacity], ["Remaining", p.available]].map(([label, value]) => <div key={String(label)}><dt className="text-sm text-ink-soft">{String(label)}</dt><dd className="mt-2 font-display text-3xl lg:text-4xl" data-metric={String(label)}>{formatDeposit(value as bigint)}</dd><p className="mt-1 text-xs text-ink-soft">ccUSD equivalent</p></div>)}</dl>
