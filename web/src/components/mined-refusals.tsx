...
export function MinedRefusals({ claimId }: { claimId: string }) {
  ...
  return <section className="panel p-5 sm:p-7"><div className="flex flex-wrap justify-between gap-3"><h2 className="font-display text-2xl">Reverted transactions</h2><span className="text-xs text-ink-soft">Live receipt verification</span></div>
    ...
    {!query.isError && query.data?.map(row => <article key={row.tx} className="mt-6 border-t border-line pt-5" data-testid="mined-refusal"><p className="text-sm font-medium text-cut">Draw refused on-chain</p><p className="mt-3 text-sm leading-relaxed">Lender {shortAddress(row.lender)} requested {formatDeposit(row.requested)} ccUSD. Remaining capacity at the mined block: {formatDeposit(row.available)} ccUSD equivalent.</p><dl className="mt-4 flex flex-wrap gap-x-8 gap-y-2 text-xs text-ink-soft"><div><dt>Status</dt><dd className="mt-1 text-ink">Mined / Reverted</dd></div><div><dt>Contract check</dt><dd className="mt-1 font-mono text-ink">{row.errorSignature ?? row.reason}</dd></div><div><dt>Block</dt><dd className="mt-1 text-ink">{row.block.toString()}</dd></div></dl>{row.rawSelector && <p className="mt-3 text-xs leading-relaxed text-ink-soft">Raw revert selector returned by the chain: <code className="font-mono text-ink">{row.rawSelector}</code> — independently verifiable as <code className="font-mono text-ink">keccak256(&quot;{row.errorSignature}&quot;).slice(0, 4)</code>, not just this app&rsquo;s own reading of it.</p>}<a href={CC3_EXPLORER_TX(row.tx)} target="_blank" rel="noreferrer" className="mt-5 inline-block text-sm underline underline-offset-4">View reverted transaction on Blockscout</a></article>)}
