...
// Gate 3 parity: a real Sablier stream with cancelable=true, so instantiateClaim's
// CancelableNotAllowed check can be exercised against a genuinely successfully-verified
// proof, not just a tampered field. Otherwise identical to scripts/create-sablier-stream.mjs.
...
const depositAmount = 1_000n * 10n ** 18n; // small -- this stream is never meant to back real capacity
...
const params = {
  ...
  depositAmount,
  token: DEMO_ASSET,
  cancelable: true, // <-- the one bad-shape field under test
  transferable: true,
...
console.log("Simulating createWithTimestampsLL (cancelable=true)...");
...
const evidence = {
  purpose: "bad-shape (cancelable=true) fixture for Gate 3 on-chain parity test",
...
writeFileSync(new URL("./bad-shape-stream.json", `file://${outDir}/`), JSON.stringify(evidence, null, 2));
console.log("\nWritten to data/gate3/bad-shape-stream.json");
