...
const startTime = now;
const cliffTime = now + 48 * 60 * 60; // 48 hours -- covers the full Gate 8 build/test window
...
const evidence = {
  purpose: "Gate 8 SECOND demo position -- for live interactive browser testing, untouched by the integration script",
...
writeFileSync(new URL("./demo-position-2-live-test.json", `file://${outDir}/`), JSON.stringify(evidence, null, 2));
console.log("\nWritten to data/gate8/demo-position-2-live-test.json");
