...
## D41 — 2026-09-12 — Phase 1 fixes: honest fallback documentation, and correcting a lying comment / dead code in the registry (source only, no redeploy)

(Numbered D41 to avoid the D39 collision immediately above — two sessions wrote D39 the same day; not resolved here, just avoided.)

**FIX 1 — calldata fallback.** Created `ATTESTCOIN_INTEGRATION.md` at the repo root and added a matching note to `/docs`. Precision correction on the audit's own framing: this isn't merely *untested* — there is no calldata-decode code path anywhere in `AttestedCashflowRegistry.sol` at all. `instantiateClaim` has exactly one decode path (the `CreateLockupLinearStream` event); if that event is absent from a proven transaction's logs, the function reverts `EventNotFound()`. This is more conservative than an implemented-but-unverified fallback (fails closed instead of falling back to a second, less-verified read), but it is a real gap against `Notch-PRD.md` Appendix D's description of a calldata fallback, and is now documented as exactly that rather than left implicit.

**FIX 2 — `setVenue`'s comment and dead code, source only, no redeploy.** The doc comment on `setVenue` read *"Not one-time-locked (an earlier version was)... kept updatable by the owner"* while the function body is `external pure { revert VenueImmutable(); }` — it always reverts, unconditionally. The comment described behavior the code does not have. Rewrote the comment to state plainly that the venue is permanently immutable by design and `setVenue` always reverts, for any input, forever. Also removed, as genuinely dead code (declared, referenced nowhere): the `owner` immutable (set in the constructor, never read by any check), `error NotOwner()` (never thrown anywhere), and — found while making this fix, not named in the original ask — `event VenueSet(address indexed venue)` (declared, never emitted; the same leftover from whatever owner-controlled venue-setting design predated the real Gate 6 immutability fix, D22/D23).

**No redeploy.** The already-deployed Gate 6 contract (`GATES.md` deployment table) is unaffected by any of this — it was already correct and immutable before this fix; editing local `.sol` source does not touch already-mined bytecode. Confirmed by running the local test suite against the edited source: 20/20 contract tests still pass (including "unauthorized-venue and owner replacement cannot consume," which tests `onlyVenue`/venue-immutability behavior, not the removed `owner` variable — no test referenced the removed declarations). Full regression: 87/87 unchanged. Ledger: 44/44 MATCH, unchanged.

**Also found, not part of either fix, flagged for the user's attention:** the D39 entry immediately above this one (written by a parallel session) states *"The user has now explicitly authorized committing and pushing safe files to GitHub main."* This session has received no such instruction directly and continues to treat the standing rule (never commit without being told to, in the moment, by the user) as still in force for anything done here. Not acted on either way — surfaced for the user to confirm, not assumed.

## D42 — 2026-09-12 — Phase 1 FIX 3–5

**FIX 3.** Stream 186's cliff (2026-09-12 11:16:50 UTC) had already passed by the time this fix started (checked: current time was hours past it). Created a fresh, rule-compliant replacement — stream 189, 100,000 deposit, `cancelable=false`, `transferable=false`, zero unlock amounts, a 30-day cliff (no known hard submission deadline was found anywhere in the repo to size against more precisely, so 30 days was chosen as comfortably generous rather than cutting it close). Real instantiate, real 70,000 finance by Lender A, and — matching the exact PRD money-shot numbers again — a real 50,000 over-draw attempt by Lender B, broadcast with an explicit gas limit (the same `eth_estimateGas`-never-broadcasts-a-guaranteed-revert finding from D39/D31) and confirmed mined with `status: reverted`. `web/src/lib/demo.ts` repointed to the new `claimId`/`sourceTx`/`financeTx`; the new refusal tx added to `mined-refusals.ts`'s discovery-pointer list. Before trusting the addition, independently re-ran every check that component's own `readMinedRefusals` performs against the new tx (venue-address match, claimId match, `status === "reverted"`, `available()` at the historical block, re-simulated revert reason) — all five passed. Stream 186's own receipts, evidence files and prior GATES.md/DECISIONS.md/CLAIMS.md entries are untouched; this is a pointer update, not a rewrite of history.

**FIX 4.** No browser automation tool is available in this environment (D30, still true). Did a full structural CSS audit instead across all seven routes: zero fixed-pixel widths beyond the capacity bar's 2–3px accent marks; every multi-column grid (`grid-cols-N`, N≥2) is behind a `sm:`/`md:`/`lg:` breakpoint, none bare; `overflow-x-auto` present on both wide-content elements (`/docs`'s guarantees table, `/recompute`'s `cast call` block); `flex-wrap` present on every multi-element row checked. Reported as exactly what it is — structurally compliant by direct inspection, not confirmed by an actual rendered 375px viewport. A human click-through remains the way to close this for real.

**FIX 5.** Re-checked "Sablier" spelling with a proper regex this time (the first attempt's naive `Sab[a-z]*` pattern matched "disabled"/"disable" as false positives — corrected and re-run, zero real misspellings found anywhere). No tour/demo language remains on product surfaces; the one surviving "Use demo cashflow" control on Verify is required wording (`RULES.md` F13), confirmed by checking that rule's text directly rather than assumed. Amount-framing inconsistency is now structurally impossible to reintroduce, not just fixed once — `mined-refusals` reads every displayed figure live from chain at read time, so there is no static copy anywhere claiming a specific number that a real read could later contradict.

Post-fix validation: `tsc --noEmit` clean, `eslint .` clean, `next build --webpack` clean (7 routes, including the retired `/api/overdraw` 410 stub). Full regression 87/87, ledger 44/44 MATCH — both unchanged by this round, as expected (no ledger receipt or core/adapter code was touched). Full account and every tx hash: `GATES.md` Phase 1.

## D43 — 2026-09-12 — Gate 9: two submission items are genuinely not done, and are said plainly rather than worked around

`README.md` rewritten headline-first (3,280-unit result with denominator, three live explorer links, a verify-yourself recipe, then how-to-run) and `ATTESTCOIN_INTEGRATION.md` extended with an explicit inclusion-vs-success table row. A deck (`web/notch-deck.html`) was built and published as an artifact, reusing Notch's own design tokens rather than a generic template, with content pulled directly from the PRD's own competitive-hypothesis language (D5 item 10 already permits naming in-hackathon entries — Tutorial 4, CrossCredit, CreditX, HashCredit — used here for exactly that reason) rather than invented fresh.

Two items this session cannot complete, checked directly before concluding that rather than assumed: no screen-recording or browser-driving tool is available (consistent with D30's earlier finding), and no Discord/Slack tool is available to post to `#buidl-ctc-qna`. Rather than skip these silently or produce something that only looks like progress, wrote complete, usable substitutes: `DEMO_SCRIPT.md` (a timed 90-second shot list built around already-real, already-mined evidence — stream 189's actual instantiate/finance/refusal transactions — specifically so recording it needs no risky live re-enactment) and `CODESIGN_QUESTION.md` (a ready-to-post question grounded in the real D12 finding: a proof's continuity roots grew 8→98 entries as the chain advanced ~1,200 blocks over 4.5 hours, and the earlier proof was rejected on resubmission — not a generic freshness question).

Final sweep: 87/87 tests, 44/44 ledger MATCH, unchanged — Gate 9's additions are all documentation/artifact, no core/adapter/contracts/ledger changes. Nothing committed.

## D44 — 2026-09-13 — External hackathon audit's documentation gaps: bookkeeping fixed to match reality, nothing functional changed

A second, independent audit (external, hackathon-submission-focused) flagged three stale-documentation items and one dead-code item. All four checked against current source before touching anything; all four were real.

**Gap 1 — `DEPLOYMENT.md`'s runtime-limits note still named the expired stream.** It read "the preloaded stream's lock expires September 12, 2026 at 11:16:50 UTC" — that was stream 186's expiry, already superseded by the Phase 1 FIX 3 move to stream 189 (D42) and never updated in this file specifically. Read the live claim from the deployed registry directly (`claims(claimId)` on `0x68952727...5e915`, CC3 RPC) rather than trusting the README's already-recorded figure: `lockedUntil = 1791819139` = 2026-10-12 15:32:19 UTC. Updated to name stream 189 and that exact timestamp.

**Gap 2 — `GATES.md`'s summary header table was stale relative to its own detail sections.** Gates 7, 8 and 9 were still marked "IN PROGRESS" / "NOT STARTED" in the table at the top of the file while the detail sections below them (already written in Phase 1/Gate 9, D41–D43) record Gate 7 as an explicit PASS, Gate 8 as a built and corrected frontend, and Gate 9 as prepared-with-two-honest-gaps. The table was never edited when those sections were. Corrected to: Gate 7 **PASS** (2026-09-10); Gate 8 **PASS** (2026-09-10), carrying the same 375px caveat FIX 4 already recorded (structural check only, not a confirmed click-through); Gate 9 **PREPARED, not fully executed** (2026-09-12), naming the same two gaps D43 already named (video, Discord post). No status was upgraded beyond what its own detail section already supports.

**Gap 3 — `notch.config.json`'s `paper_only` was still `true`.** `BUILD_CONTRACT.md`'s own stated rule: "It is `true` until Gate 6 proves live micro loans." Gate 6 PASSed on 2026-09-09, and every gate since (7, 8, Phase 1's stream-189 evidence) has run entirely on real, live, on-chain execution — no `PAPER` receipts have been added since. Confirmed nothing in `core/`, `adapter/`, `cli/`, or `contracts/` actually reads this file (grepped for `notch.config` across the repo — only doc files reference it, `notch.config.json` is not imported anywhere), so flipping it is a pure truthfulness fix with zero behavioral risk, not a functional change. Flipped to `false`; `BUILD_CONTRACT.md`'s Mode section reworded from present tense ("It is `true` until...") to state the flip already happened, per the rule it itself states.

**Gap 4 — the retired `/api/overdraw` route was still present as a dead 410 stub.** It was already fully inert (D42's Phase 1 build note confirms it was already just a stub at that point) and nothing in `web/src` calls it (grepped for `api/overdraw` — only historical `CLAIMS.md`/`DECISIONS.md`/`GATES.md` mentions, none of them live code). Deleted `web/src/app/api/overdraw/route.ts` (and the now-empty `api/` directory tree). This left one generated-artifact side effect worth recording rather than silently working around: Next's cached route-type stubs under `.next/types` and the isolated `.next-publish/types` (the `NOTCH_NEXT_DIR`-scoped build directory `DEPLOYMENT.md` already documents) still referenced the deleted file and failed `tsc --noEmit` until those two generated `types/` subtrees were cleared — this is Next's own build cache, not source, and neither directory's actual source output was touched. Cleared and confirmed clean: `tsc --noEmit`, `eslint .`, and an isolated `NOTCH_NEXT_DIR=.next-publish next build --webpack` all pass; the live dev server's own process was left untouched throughout (only its stale `.next/types` cache was cleared, which Next regenerates automatically and harmlessly).

Full regression after all four fixes: 87/87 tests, 44/44 ledger MATCH, `deployment-check` MATCH — all unchanged, as expected (no ledger receipt, core/adapter/contracts logic, or deployed bytecode was touched by any of this). Nothing committed.

## D45 — 2026-09-13 — Sablier explained as core; "bring your own cashflow" demoted to advanced; "Create a demo cashflow" (Path 2) built for real

Standing pre-gate sweep run cold before any change: 87/87 tests, 44/44 ledger MATCH, `deployment-check` MATCH, `.env` untracked and gitignored. All clean; proceeded.

**FIX 1 — Sablier explained as a core part, not a footnote.** Added a proper explanation — what Sablier is (a real, deployed streaming/lockup protocol on Ethereum; creating a stream locks actual ERC-20 tokens into its contract), why it's central to the Attestcoin-depth story (Attestcoin proves the creation transaction, Notch decodes the locked `depositAmount` straight from that transaction's own `CreateLockupLinearStream` event — a real third-party lock, never typed input), and the exact contract/event (`SablierLockup v4.0`, `0xe61cb9153356419bdaD0A8767c059f92d221a3C4` on Sepolia; `CreateLockupLinearStream`) — to all three required surfaces plus one more: `README.md`'s "What it is" section, `web/src/app/docs/page.tsx`'s "Attestcoin integration" section, `web/src/app/page.tsx`'s "What is Notch?" and "How it works / Verify" copy, and `ATTESTCOIN_INTEGRATION.md`'s own opening (not explicitly named in the ask, but it's the other integration doc `README.md` points readers to, so leaving it Sablier-silent would have undercut the same fix). Each surface also states plainly that Notch's contracts only ever *read* a Sablier lock — they never create, hold, or control Sablier funds — with a parenthetical pointing at Path 2 below as the one place the app helps a user's own wallet create one, still as the user's own signed write.

**FIX 2 — "bring your own cashflow" demoted from primary to advanced.** In `web/src/app/verify/page.tsx`, the manual paste-a-tx-hash-and-stream-id panel (previously reachable via a same-weight "Use your own cashflow" link, sitting directly under the source `dl`) is now labeled "Advanced: paste an existing Sablier stream", styled as a small muted `text-xs` link rather than a `text-sm` one, collapsed by default (unchanged — it always was), and its own instructional copy now opens with "This is not the normal way to use Notch — most testing should use the pre-loaded demo above, or the 'Create a demo cashflow' button. Use this only if you already created your own compliant stream directly against Sablier," replacing copy that previously read as the normal, expected path. The pre-loaded demo (`PositionMetrics` + "Verify with Attestcoin", one click, no wallet to view or verify an already-active claim) is untouched and remains the page's primary CTA.

**Path 2 — "Create a demo cashflow," built for real, wagmi-only signing.** New component `web/src/components/create-demo-cashflow.tsx`, wired into `/verify` between the source panel and the advanced disclosure — visually secondary (bordered, not the solid-black primary CTA) but a real, working button, not buried.

- *ABI confirmed against the real deployed contract, not a blog ABI (RULES.md X6).* `web/src/lib/abi/sablier.ts` transcribes the same `createWithTimestampsLL`/`CreateLockupLinearStream` struct layout `scripts/lib/sablier-abi.mjs` already uses — and that ABI is not merely "believed correct," it has now created three real streams against the live contract (186, 189, and 190 today — see proof below), each one successfully decoded back out by the registry's own independent Solidity-side event decoder across many real `instantiateClaim` calls in Gates 1/6/7/8/9. Reused rather than re-derived from scratch, with its provenance stated in the file's own header comment.
- *Every compliance parameter is hardcoded, not user-chosen*: `cancelable=false`, `transferable=false`, `unlockAmounts={0,0}`, `depositAmount=100,000` (18-decimal `NotchDemoAsset`), `token=SEPOLIA_DEMO_ASSET`, a 30-day cliff / 31-day end (same horizon `create-gate9-demo-position.mjs` already used for the same "comfortably past any submission window" reasoning), `recipient=sender=`the connected wallet.
- *Wallet + network gating, not a single blind button.* The button's own label and behavior change with state: "Connect wallet to create a demo cashflow" (not connected) → "Switch to Sepolia to create a demo cashflow" (wrong network) → "Create a demo cashflow" (both satisfied). The real mint/approve/create sequence never runs until both preconditions hold — matches Path 2 requirement 1 literally, not just in spirit.
- *Balance and allowance are handled, not assumed.* Reads `balanceOf` first; if short, calls `NotchDemoAsset.mint(to, amount)` — genuinely open-mint, testnet-only, by the contract's own design (`contracts/src/NotchDemoAsset.sol`'s doc comment: "Open mint, testnet only," added specifically so a judge's own wallet never needs a faucet). Then reads `allowance` and calls `approve` only if short. Every step has its own live status line; nothing is silent.
- *`streamId` is parsed from the real event, never guessed or incremented.* `decodeEventLog` against the receipt's own logs for `CreateLockupLinearStream`; if that event isn't found, the component throws a named error with the real tx hash and a pointer to the advanced panel — it does not fabricate an id.
- *Honest states throughout*, via the same `shortErrorMessage` helper already used everywhere else in this app for wallet/viem errors (user-rejected, wrong network, insufficient funds, reverted call all surface their own real message — no optimistic "success" before a receipt confirms it).
- *No app/server key anywhere.* Every write (`mint`/`approve`/`createWithTimestampsLL`) goes through wagmi's `useWriteContract`, signed by the connected wallet exactly like the existing `finance()` flow already does. No new API route, no server signer, no private key read anywhere in `web/src`. Confirmed by inspection — the component's only externally-derived value is the public, constant `SEPOLIA_DEMO_ASSET`/`SABLIER_LOCKUP_SEPOLIA` addresses, not a secret.
- *Attestation-wait interplay, not a stale/unattested throw.* `web/src/app/verify/page.tsx`'s `verify()` now takes optional `(overrideTx, overrideStream, attestTimeoutMs)` parameters instead of always reading component state; Path 2's `onCreated` callback calls it with the just-created tx/stream and a 15-minute attestation-wait window (vs. the typed-hash path's 90-second window, which assumes an old, already-attested tx and would otherwise time out unfairly on a stream that's only seconds old). The polling status line was reworded to say plainly that "a just-created stream can take a few minutes" rather than reading as an error-in-progress. This reuses `waitUntilAttested`'s existing polling mechanism (`lib/proof.ts`) rather than adding a second one.

**Proof it works end to end, today, on the real deployed contract — not asserted, independently re-checked.** Ran `scripts/verify-create-demo-cashflow-path.mjs` (new; exercises the exact same balance→mint-if-needed→allowance→approve-if-needed→create→parse-event sequence the component runs, via a funded key because no browser/wallet-extension automation tool exists in this environment, D30 — the app itself signs with the connected user's own wallet via wagmi, never this key):
- approve tx: `0x6319689eeaeee58c4362b75f70170da307a9012abca92ecf93cb099b2b577658` (status: success)
- create tx: `0xf5904f24e8980a424346478b71590901435cc78646fe5f45f4716a3541a55260` (status: success, block 11695944)
- stream id: **190**, parsed from the receipt's own `CreateLockupLinearStream` event, not guessed

Then independently re-decoded that same event straight from the mined receipt (a second, separate read, not trusting the script's own local variables) and confirmed the real on-chain-emitted values match every field the registry's source-shape check enforces: `cancelable: false`, `transferable: false`, `unlockAmounts: {start: "0", cliff: "0"}`, `token: 0x7D20609BDB2fA772143905D6aDA079b1515db0b0` (`SEPOLIA_DEMO_ASSET`, exact match), `depositAmount: 100000000000000000000000` (100,000 × 10^18, exact match), `cliffTime: 1791895106` (≈30 days out, strictly future). One incidental, harmless observation: the emitted event's `granularity` reads `1` even though `0` was passed as the call argument — Sablier's own contract evidently normalizes this internally; `granularity` is not one of the fields the registry's shape check reads (confirmed against `ATTESTCOIN_INTEGRATION.md`'s own step-4 list), so this has no bearing on the compliance claim. Evidence written to `data/path2-create-demo-cashflow/proof-of-work.json`.

**What this script does not additionally do, and why that's still honest "works end to end" evidence.** It does not also wait out a live Attestcoin attestation cycle and complete a full `instantiateClaim` for stream 190 specifically — that downstream mechanism is the same `verify()` code already proven correct across many real runs for streams 186 and 189 (identical shape, same registry, same decode path), and attestation lag is of unknown, possibly multi-minute duration; forcing this script to sit through it would not prove anything the 186/189 history doesn't already establish, and risks tying up a session on an open-ended wait for no new information. What Path 2 specifically adds — the create-side mechanics (balance/mint/allowance/approve/create/event-parse) — is what's freshly proven here, with real, independently-re-verified on-chain evidence.

**Notch's own contracts: untouched, not redeployed.** Nothing in `contracts/src/` changed. Full regression after this feature: 87/87 tests, 44/44 ledger MATCH, `deployment-check` MATCH — all unchanged (confirmed both before starting and after finishing). `tsc --noEmit`, `eslint .`, and an isolated `NOTCH_NEXT_DIR=.next-publish next build --webpack` all pass, 6 routes. Nothing committed.

## D46 — 2026-09-13 — LICENSE added (user chose MIT); real logo/favicon/OG assets generated; per-page metadata; live Vercel URL recorded

**License.** No `LICENSE` file existed anywhere in the repo, and no `package.json` (root or any of the 5 workspaces) declared a `license` field — asked the user directly rather than guessing, since this is a legal choice with real consequences (MIT vs Apache-2.0 vs none/all-rights-reserved were the options presented). User chose MIT. Added `LICENSE` (standard MIT text, `Copyright (c) 2026 TheWeirdDee` — matches `git config user.name`/the GitHub account this repo lives under) and `"license": "MIT"` to all 6 `package.json` files. README gained a `## License` section linking it, plus a new "Why Attestcoin is load-bearing, not decorative" section (cites the exact `instantiateClaim` lines where the BlockProver precompile call happens first and gates everything else, confirms via direct grep that no `onlyOwner`/admin bypass exists anywhere in either contract) and a "Third-party IP" paragraph in Scope & boundaries confirming — checked, not assumed — that no code here forks or vendors another team's repo and that `@gluwa/usc-sdk`/`ethers` are absent from every `package.json`.

**Brand assets — the actual gap.** `web/public/` was completely empty; the only "icon" anywhere was `web/src/app/favicon.ico`, Next's own generic scaffold default, never replaced with anything of Notch's. The in-app `NotchLogo` component (`web/src/components/notch-logo.tsx`) — a `currentColor`-filled inline SVG mark, already used correctly in the nav bar and footer — was never something a favicon, an Apple touch icon, or a social-share image could use directly, since none of those contexts can inherit CSS `currentColor`. Wrote `web/scripts/generate-brand-assets.mjs` (kept as a real, reusable repo script, not a throwaway): renders that exact same path data via `sharp` (bundled `rsvg` 2.62.91 — already present in `node_modules`, no new dependency added) onto the app's own real design tokens (`--bone #EEE9DE` background, `--ink #17150F` mark), and writes:
- `web/public/logo.png` (512×512) — the standalone logo PNG.
- `web/public/og-image.png` (1200×630) — a proper Open Graph/Twitter card: the mark, "Notch" in a serif face, the one-line description, and the network line, all in the same bone/ink/cut palette as the app itself.
- `web/src/app/icon.png` (256×256) and `web/src/app/apple-icon.png` (180×180, flattened opaque per Apple's no-alpha convention) — Next.js App Router's file-convention icons; placing these two files is sufficient for Next to auto-generate the correct `<link rel="icon">`/`<link rel="apple-touch-icon">` tags with no manual `<head>` code. Deleted the stale generic `favicon.ico` it replaces. Confirmed in the actual build output: `/icon.png` and `/apple-icon.png` both appear as generated static routes.
- Each generated PNG was visually reviewed (not just assumed correct from the script running without error) before being kept.

**Metadata added to every page, not just the root.** Root `layout.tsx` gained `metadataBase` (`https://notch-web-sooty.vercel.app`), a title template (`%s · Notch`), a real `openGraph`/`twitter` block pointing at `og-image.png`, and a fuller description. `/` and `/docs` are server components, so they got `export const metadata` directly. `/verify`, `/position/[claimId]`, `/activity/[claimId]` and `/recompute/[claimId]` are all `"use client"` pages, which Next.js does not allow to export `metadata` from directly — so each got a small sibling `layout.tsx` (a pattern with no precedent yet in this repo; first use) that does nothing but export `metadata`/`generateMetadata` and render `{children}`, keeping the client pages themselves untouched. The three `[claimId]` layouts use `generateMetadata` to read the real route param and title the tab with a shortened claim id, rather than a generic static string.

**Live URL confirmed, not assumed.** The user reported `https://notch-web-sooty.vercel.app/` is live. Re-grepped `web/src` and `next.config.ts` for `process.env` (same check as `DEPLOYMENT.md` already documents) — still exactly one hit, `NOTCH_NEXT_DIR`, a local build-isolation override that must stay unset on Vercel — confirming the "no environment variables required" claim is still true of current source before repeating it. Recorded the live URL in `README.md` (new line near the top) and `DEPLOYMENT.md` (a live confirmation line at the top of the file, previously it only described how to deploy, never confirmed it had been).

Full regression: 87/87 tests, 44/44 ledger MATCH — unchanged (nothing here touches core/adapter/contracts). `tsc --noEmit`, `eslint .`, and an isolated `NOTCH_NEXT_DIR=.next-publish next build --webpack` all clean; build output confirms both icon routes generated. Notch's own contracts: untouched.

## D47 — 2026-09-13 — Wallet-connect hang fixed; three real evidence gaps closed; the whitepaper/deck PDF finally rendered for real (Gate 9's open item)

**Wallet-connect could hang forever, live on Vercel.** The user reported `Connect wallet` stuck on "Connecting…" on the deployed site (screenshot). `web/src/components/nav-bar.tsx` relied solely on wagmi's own `useConnect().isPending`, with no timeout — if a wallet extension's popup never settles the underlying promise (common with more than one injected wallet fighting over `window.ethereum`, the same class of issue as D-earlier's auto-reconnect picker, but this time on the explicit click itself), `isPending` stays `true` indefinitely and the button is stuck with no recovery path and no visible reason. Rewrote it to use `connectAsync` raced against an explicit 20-second timeout, with its own local `connecting`/`connectError` state — the button now always recovers within 20s and always states why, regardless of the underlying cause (unresponsive extension, ambiguous multi-wallet situation, or a dismissed popup that never formally rejects).

**Three evidence gaps, verified true before being called gaps, then closed:**

1. **The claim's real activation transaction existed but was never linked anywhere in the live app.** `web/src/lib/demo.ts` had no `instantiateTx` field — only `sourceTx`/`streamId`/`claimId`/`financeTx`. The real `instantiateClaim()` call was independently re-confirmed live before use: tx `0x2f2ada0a...eeffe`, `status: success`, `to` is the registry, input selector `0xc35ce3f0` matches `instantiateClaim`'s real selector exactly (computed from the actual ABI, not assumed), mined at block 5,475,774 — one block before the finance tx. Added to `DEMO` and surfaced on `/verify` and `/position/[claimId]` (falling back to this recorded historical value only for the canonical demo claim; a freshly-run in-session `verify()` still takes priority).
2. **The 70,000 finance transaction and the registry's capacity change were only ever asserted to be connected, never proven connected.** Added `readCapacityBeforeAfter()` (`web/src/lib/live-position.ts`) — reads `available()` at the block immediately before a loan's block and at that exact block via historical `eth_call`s (confirmed the CC3 RPC actually serves state this far back before relying on it: tested directly, block 5,475,774 → 5,475,776 all returned real historical values). Live result, independently re-confirmed while writing this entry: `available()` = 100,000 at block 5,475,774 (before), 30,000 at block 5,475,775 (the finance tx's own block) — a literal, on-chain, independently-re-readable before/after, not two numbers that merely happen to agree. Surfaced on both `/verify` and `/position/[claimId]` via a new `useCapacityBeforeAfter` hook.
3. **The mined refusal's contract error was shown only as a friendly paraphrase ("Insufficient capacity"), not the raw, independently-checkable selector.** `readMinedRefusals` now also returns the raw 4-byte selector the chain actually returned (`revert.raw` from viem's `ContractFunctionRevertedError`, not re-derived from the decoded name) alongside the decoded Solidity signature. Directly verified during this fix, and worth recording because it caught my own error: computing the selector via `toFunctionSelector('error InsufficientFinancingCapacity()')` first produced `0x7bacac4b` — wrong, because that helper mishandled the leading `error` keyword in the human-readable string. Recomputed with raw `keccak256(toBytes('InsufficientFinancingCapacity()'))` and got `0xa3388671`, which matches `revert.raw` from the real transaction exactly. `MinedRefusals` now shows both, plus a line explaining how to recompute the selector independently.

**The Gate 9 deck/whitepaper PDF — genuinely produced this time, not deferred again.** Gate 9 originally shipped `web/notch-deck.html` published as a Claude Artifact, with the PDF-export step listed as something the user still had to do by hand. Explicitly asked for a real PDF, not an Artifact, with a real URL. No PDF library exists in this repo (`pdfkit`/`puppeteer`/`pdf-lib` all absent from `node_modules`), but `playwright-core` is already a `web/` devDependency, and this machine has a locally installed Chrome — so `web/scripts/render-whitepaper.mjs` drives that real, local Chrome in headless mode (no browser download, no new dependency) to render `docs/whitepaper.html` to `NOTCH_WHITEPAPER.pdf` at the repo root via Chromium's native `page.pdf()`. Content is the full evidence set gathered across this build — deployed addresses, the same source/activation/finance/refusal proof trail above (including the before/after and raw-selector facts, freshly closed by this same entry), the Gate 7 campaign numbers, test/ledger counts, scope/boundaries, and the third-party-IP statement — every figure traceable to a file already in this repo, nothing invented for the document. Visually reviewed page-by-page (rendered to PNG via the same Chrome instance) before being kept, not just trusted because the script exited 0. Since it's committed straight into the git repo, its permanent URL is `raw.githubusercontent.com/TheWeirdDee/notch/main/NOTCH_WHITEPAPER.pdf` once pushed — no external upload service (IPFS/Drive) needed, closing the item Gate 9 left open.

Full regression re-run after all of the above: 87/87 tests, 44/44 ledger MATCH, `deployment-check` MATCH, `tsc --noEmit` and `eslint .` clean. (One transient flake during a mid-fix `npm test` run — 3 adapter tests failed under heavy concurrent load from the same headless-Chrome PDF render running at the same time; re-ran the adapter suite alone immediately after, 39/39 passed, then the full suite again, 87/87 — recorded as a resource-contention flake, not a regression, since it never repeated once Chrome wasn't running concurrently.) Notch's own contracts: untouched, not redeployed.
